...
class ItemListApproveScientificProvider extends ChangeNotifier {
  final clinicActivityService = getIt<ClinicActivityService>();
  final List<Clinic> listScientific = [];
  bool loading = false;

  ItemListApproveScientificProvider() {
    getListScientific();
  }
  ...
  void getListScientific() async {
    ...
    notifyListeners();
    final result =
        await clinicActivityService.getListClinic(status: 1, idFlow: 2);
